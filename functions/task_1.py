...
def task_1(folder="tutorial", output1="integers_a.csv", output2="integers_b.csv"):
    try:
        faasr_log("Starting task_1: generating random integer files")
        ...
        faasr_log(f"Written local file {local_path_a} with {len(integers_a)} rows")

        with open(local_path_b, 'w') as f:
            for val in integers_b:
                f.write(f"{val}\n")

        faasr_log(f"Written local file {local_path_b} with {len(integers_b)} rows")
