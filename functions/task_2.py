...
def task_2(folder='tutorial', input1='integers_a.csv', input2='integers_b.csv', output1='summed_results.csv'):
    faasr_get_file(remote_folder=folder, remote_file='integers_a.csv', local_file='integers_a.csv')
    faasr_get_file(remote_folder=folder, remote_file='integers_b.csv', local_file='integers_b.csv')
    ...
    with open(path_a, 'r', newline='') as f:
        ...
        for row in reader:
            val = row['value'].strip()
            try:
                values_a.append(int(val))
            except ValueError:
                raise ValueError(f"Cannot parse '{val}' as a whole integer in {input1}")
    values_b = []
    with open(path_b, 'r', newline='') as f:
        ...
        for row in reader:
            val = row['value'].strip()
            try:
                values_b.append(int(val))
            except ValueError:
                raise ValueError(f"Cannot parse '{val}' as a whole integer in {input2}")
    ...
    faasr_put_file(local_file='summed_results.csv', remote_folder=folder, remote_file='summed_results.csv')